...
class Workflow:
    def __init__(self, steps: OrderedDict[str, tuple[str, Callable[..., dict]]], console: rich.console.Console, logger: logging.Logger, **global_args):
        """
        steps: dict of step_id -> (step name, executor function)
        global_args: any kwargs to pass to each executor
        """
        self.steps = {key: Step(name, func) for key, (name, func) in steps.items()}
        ...
        with Live(self.render(), console=self.console, refresh_per_second=10) as live:
            for key, step in self.steps.items():
                ...
                try:
                    ...
                    kwargs = {**accumulated_output, **self.global_args}
                    result = step.func(**kwargs)
                    step.output = result or {}

                    collision_keys = set(accumulated_output).intersection(step.output)
                    if collision_keys:
                        raise ValueError(
                    ...
                    live.update(self.render())
                    self.logger.exception(f"Step '{step.name}' failed")
                    raise e
...
def render_comment(comment: Comment, logger: logging.Logger, use_ruler: bool = False):
    ...
    filtered_diffs = [diff for diff in parsed_output if diff.new_start_line_idx + 1 <= comment.line_no < diff.new_start_line_idx + diff.new_len + 1]
    panel = Panel(comment.body, title=f"{comment.relative_path}:{comment.line_no}", style=Style(bold=True, bgcolor="black"))
    ...
    assert len(filtered_diffs) == 1, "Found multiple diffs containing the same lines, this should not happen"

    diff = filtered_diffs[0]
    diff_str_lines = []
    ...
    # TODO: customize the panel as needed.
    ...
    for change in diff.changes:
        ...
        if change.new_line_idx is not None:
            if curr_new_line == comment.line_no:
                syntax = Syntax("".join([diff_header]+ diff_str_lines[-window_size:]), "diff", theme="ansi_dark")
                yield syntax
                yield panel
                diff_str_lines = []
    ...
    if diff_str_lines:
        syntax = Syntax("".join(diff_str_lines[:window_size]), "diff", theme="ansi_dark")
        yield syntax
...
def display_comments(comments: list[Comment], console: rich.console.Console, logger: logging.Logger):
    comment_groups = defaultdict(list)

    for comment in comments:
        comment_groups[comment.relative_path].append(comment)

    for rel_path, file_comments in comment_groups.items():
        file_comments = sorted(file_comments, key=lambda x: x.line_no)
        use_rulers = [True for _ in range(len(file_comments))]
        use_rulers[-1] = False
        file_gp = Group(*(render_comment(comment, logger=logger, use_ruler=use_ruler) for comment, use_ruler in zip(file_comments, use_rulers)))
        file_panel = Panel(file_gp, title=f"[bold white on blue] {rel_path} [/]")

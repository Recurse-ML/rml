...
from rml.package_config import RECURSE_HOSTNAME
...
def post_check(archive_filename: str, archive_path: Path, target_filenames: list[str]) -> None:
    try:
        post_response = httpx.post(
            f"http://{RECURSE_HOSTNAME}/api/check/",
